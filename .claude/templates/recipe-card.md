...
| **Configure** | Engine artifact (collection/view/automation/form) or manifest keys | — | new keys: section, plane, scope, tier, default+bounds per rule-config-framework |
...
- **Seams declared**: the join patterns this unit participates in (e.g. sign-in + audit + UI; integration + automation + notification; permissions + Engine views). Every declared seam must map to a seam suite in §5A — a unit that names no seams is either trivially isolated or wrongly analysed; say which.
...
- ☐ **Seam suites**: every join pattern declared in §4 passes its composition tests — run on the real composition, no mocked hops. Units are necessary; **seams are what we actually trust.** A new join pattern with no existing suite means writing the suite is part of this card's scope.
- ☐ **Accessibility** (any UI-touching work, per rule-accessibility §7): automated WCAG 2.2 AA scan clean on every touched surface · colour-never-sole-signal verified (incl. colour-blind simulation for palettes/charts) · new design-system components carry a11y conformance fixtures · no hardcoded user-facing strings · person-facing surfaces name their HUMAN verification (automated tools catch ~20–50% — a clean scan is necessary, never sufficient)
...
- ☐ Simulated run: named twin scenario(s) with expected outcomes — all pass
...
| # | Brief clause | Verified by (test/fixture/seam-suite/scenario id) | Status |
|---|---|---|---|
Every clause gets a row. A clause with no machine-checkable verification must be either rewritten until it has one, or explicitly marked **HUMAN-JUDGEMENT** with Dane/Jack (or the named user group, e.g. assisted-use testers) as its verifier. No row may be empty.

**C. Human gates:** the specific items from §4 one-way doors + anything on the agent red-lines list.

**D. Evidence bundle (attached on completion):** test-run links · fixture + seam-suite + a11y-scan results · screenshots/recording of the working feature · changelog entries · **exact package versions used** (Dain Bench provenance).
